...
3. **Start the docker services:**
    ...
    This will start a local instance of arrowix-dev that will store spurs and other state information in a postgres database. A local postgres service is used by default. Override `POSTGRES_*` variables in the `.env` file to use an external postgres database.
...
# 🛠️ arrowix-dev Development Setup
...
# 🦙 Using arrowix-dev with Ollama (Local Models)

arrowix-dev can work with local models served using Ollama.

Steps to configure arrowix-dev to work with Ollama running on the same host.

### 1. Configure Ollama
To ensure Ollama API is reachable from arrowix-dev, we need to start the Ollama service with environment variable `OLLAMA_HOST=0.0.0.0` . This allows requests coming from arrowix-dev docker's bridge network to get through to Ollama.
An easy way to do this is to launch the ollama service with the following command:
```sh
OLLAMA_HOST="0.0.0.0" ollama serve
```

### 2. Update the arrowix-dev .env file
...
In arrowix-dev's set up, `host.docker.internal` refers to the host machine where both arrowix-dev and Ollama are running.

### 3. Launch the arrowix-dev app
Follow the usual steps to launch the arrowix-dev app, starting with the command:
```sh
docker compose -f docker-compose.prod.yml up --build -d
```

If you wish to do arrowix-dev development with ollama please run the following command instead of above:
```sh
...
arrowix-dev only works with models that support structured-output and json mode. Most newer models should be good, but it would still be good to confirm this from Ollama documentation for the model you wish to use.
